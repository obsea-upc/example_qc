...
from m3.qc.ioos_qc import qartod
from m3.qc.ioos_qc.utils import dict_update, cf_safe_name, GeoNumpyDateEncoder
...
class QcConfig(object):
    ...
    def run(self, **passedkwargs):
        ...
        for modu, tests in self.config.items():
            try:
                #print("Importing ", 'ioos_qc.{}'.format(modu))
                testpackage = import_module('m3.qc.ioos_qc.{}'.format(modu))
            except ImportError:
