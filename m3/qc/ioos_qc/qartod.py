...
from m3.qc.ioos_qc.utils import (
    isnan,
    isfixedlength,
    add_flag_metadata
)
...
@add_flag_metadata('rate_of_change_test_quality_flag2', 'Rate of Change Test Quality Flag 2')
def rate_of_change_test2(inp : Sequence[N],
                        tinp : Sequence[N],
                        threshold : float,
                        period: float,
                        ) -> np.ma.core.MaskedArray:
    """Implementation of the RateOfChange test from the QARTOD manual.
    Returns:
        A masked array of flag values equal in size to that of the input.
    """
    with warnings.catch_warnings():
    ...
    # Save original shape
    original_shape = inp.shape
    inp = inp.flatten()  # Return a copy of the array collapsed into one dimension.

    # Start with everything as passing (1)
    flag_arr = np.ma.ones(inp.size, dtype='uint8')
    df = pd.DataFrame(data={"timestamp": tinp, "value": inp, "stdevs": np.zeros(len(tinp))})
    df.set_index("timestamp", inplace=True)

    period_tdelta = pd.Timedelta(period, "H")
    init_index = df.index.values[0]
    # the first n hours (period), instead of looking backwards for the standard deviation, look forward
    df.loc[:init_index + period_tdelta, "stdevs"] = df[:init_index + period_tdelta]["value"].std()

    # compute the rest with increments of 1 hour

    t_step = pd.Timedelta(1, "H")  # calculate
    t_last = init_index + period_tdelta  # last calculated
    while t_last <= df.index.values[-1]:  # while whe are within the time ragne
        t_std_init = t_last - period_tdelta
        t_std_end = t_last
        std_value = df.loc[t_std_init:t_std_end, "value"].std()
        df.loc[t_last:t_last + t_step, "stdevs"] = std_value
        t_last += t_step

    # numpy diff: Calculate the n-th discrete difference along the given axis (if n=1,  out[i] = a[i+1] - a[i])
    roc = np.ma.zeros(inp.size, dtype='float')
    roc[1:] = np.abs(np.diff(df["value"].values))  # calculate the difference with previous value: y = x[i] - x[i-1]
    df["rate_of_change"] = roc
    df["flags"] = np.ma.ones(inp.size, dtype='uint8')
    df.loc[df["rate_of_change"] > threshold*df["stdevs"], "flags"] = QartodFlags.SUSPECT

    flag_arr = df["flags"].values
    flag_arr[inp.mask] = QartodFlags.MISSING
    return flag_arr.reshape(original_shape)
